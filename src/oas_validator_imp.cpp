...
OASValidatorImp::OASValidatorImp(const std::string& specs_path)
{
    ...
    if (!ifs.is_open()) {
        throw ValidatorInitExc("Cannot open OAS file: " + specs_path);
    }
    ...
    if (doc.HasParseError()) {
        throw ValidatorInitExc("Unable to parse specs: " + specs_path);
    }
                                               ...
                                               std::unordered_map<size_t, ParamRange>* param_idxs, std::string* query)
{
    ...
    try {
        ...
        validators = oas_validators_[static_cast<size_t>(enum_method)].per_path_validators.at(std::string::npos == query_pos ? http_path : http_path.substr(0, query_pos));
        ...
        bool found = param_idxs ? oas_validators_[static_cast<size_t>(enum_method)].path_trie.Search(beg, end, map_key, *param_idxs)
        ...
        if (found) {
            try {
                ...
                error_msg = R"({"errorCode":"INVALID_ROUTE","details":{"description": "Invalid HTTP method ')" + method + "' or path: '" + http_path + R"('"}})";
...
const std::unordered_map<std::string, HttpMethod> OASValidatorImp::kStringToMethod = {
    ...
    {"post", HttpMethod::POST},       {"put", HttpMethod::PUT},         {"delete", HttpMethod::DELETE},   {"head", HttpMethod::HEAD},     {"options", HttpMethod::OPTIONS},
